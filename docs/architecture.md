...
0. **Architecture & repository foundation** — this document, monorepo
   ...
   settings. **Done.**
1. Obsidian knowledge architecture (vault spec, templates).
2. Obsidian MCP/REST integration (`KnowledgeStore` abstraction).
3. PostgreSQL data layer (schema, migrations).
...
10. Thesis agent (explicit `THESIS_INTACT` / `STRENGTHENED` / `WEAKENED` /
    `INVALIDATED` / `INSUFFICIENT_EVIDENCE` states; changes are auditable).
11. Trading journal intelligence.
